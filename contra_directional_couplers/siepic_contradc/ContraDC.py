    ...
    **Class Attributes**: Are calculated by the different member functions during simulation.
    ...
    def getPropConstants(self):
        ...
        if self.polyfit_file is None:
            ...
            n1 = np.reshape(np.loadtxt(os.path.join(os.path.dirname(__file__), "Database", "neff", "neff_1.txt")), (5, 5, 5))
            n2 = np.reshape(np.loadtxt(os.path.join(os.path.dirname(__file__), "Database", "neff", "neff_2.txt")), (5, 5, 5))
            w1_w2_wvl = np.loadtxt(os.path.join(os.path.dirname(__file__), "Database", "neff", "w1_w2_lambda.txt"))
    ...
    def displayResults(self, tag_url=False):
        ...
        plt.grid(visible=True, color='w', linestyle='-', linewidth=1.5)
